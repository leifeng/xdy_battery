import { query, created, update, remove } from '../services/sysParamsSet'

export default {

  namespace: 'sysParamsSet',
  ...
  effects: {
    *query({args}, {select, call, put}) {
      ...
      const searchQuery = yield select(state => state.sysParamsSet.searchQuery)
    ...
    *update({args}, {select, call, put}) {
      yield put({ type: 'modalLoadingState', data: true });
      const record = yield select(state => state.sysParamsSet.record)
