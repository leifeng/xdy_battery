import { query, create, update, remove } from '../services/batteryProcessNoticeMG'
import { parse } from 'qs'

export default {

  namespace: 'batteryProcessNoticeMG',
  ...
  subscriptions: {
    ...
    //     if (location.pathname === '/admin/batteryRec/batteryProcessNoticeMG') {
    //       dispatch({
    //         type: 'query',
    //         args: {
    //           current: 1
  ...
  effects: {
    *query({args}, {select, call, put}) {
      ...
      const pageSize = yield select(state => state.batteryProcessNoticeMG.pageSize)
      const current = yield select(state => state.batteryProcessNoticeMG.current)
