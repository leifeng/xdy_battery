import { query, create, update, remove } from '../services/batteryInventoryInfo'
import { parse } from 'qs'

export default {

  namespace: 'batteryInventoryInfo',
  ...
  subscriptions: {
    ...
    //     if (location.pathname === '/admin/batteryRec/batteryInventoryInfo') {
    //       dispatch({
    //         type: 'query',
    //         args: {
    //           current: 1
  ...
  effects: {
    *query({args}, {select, call, put}) {
      ...
      const pageSize = yield select(state => state.batteryInventoryInfo.pageSize)
      const current = yield select(state => state.batteryInventoryInfo.current)
