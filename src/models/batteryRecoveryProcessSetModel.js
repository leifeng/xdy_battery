...
export default {
  ...
  namespace: 'batteryRecoveryProcessSet',
  ...
  effects: {
    *query({args}, {select, call, put}) {
      ...
      const pageSize = yield select(state => state.batteryRecoveryProcessSet.pageSize)
      const current = yield select(state => state.batteryRecoveryProcessSet.current)
