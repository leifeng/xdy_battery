...
export default {
  ...
  effects: {
    *query({args}, {select, call, put}) {
      ...
      const pageSize = yield select(state => state.serviceRunMG.pageSize)
      const current = yield select(state => state.serviceRunMG.current)
  ...
  reducers: {
    ...
    loadingState(state, action) {
      console.log('loadingState', action)
