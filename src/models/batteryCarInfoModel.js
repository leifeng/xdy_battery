...
export default {
  ...
  subscriptions: {
    setup({dispatch, history }) {
      history.listen(location => {
        if (location.pathname === '/admin/batteryBasic/batteryCarInfo') {
          dispatch({
            type: 'query',
            args: {
              current: 1
  ...
  effects: {
    *query({args}, {select, call, put}) {
      ...
      const pageSize = yield select(state => state.batteryCarInfo.pageSize)
      const current = yield select(state => state.batteryCarInfo.current)
