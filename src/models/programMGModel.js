import { query, created, update, remove } from '../services/programMG'
...
export default {
  ...
  subscriptions: {
    setup({dispatch, history }) {
      history.listen(location => {
        if (location.pathname === '/admin/sys/programMG') {
          dispatch({
            type: 'query',
            args: {
              pageSize: 10,
  ...
  effects: {
    *query({args}, {select, call, put}) {
      ...
      const searchQuery = yield select(state => state.programMG.searchQuery)
    ...
    *update({args}, {select, call, put}) {
      yield put({ type: 'modalLoadingState', data: true });
      const record = yield select(state => state.programMG.record)
