...
class BatteryRetireModalcus extends Component {
  ...
  render() {
    const {title, visible, onOk, onCancel, onClose, info, retireChild, batPakList, form, record} = this.props;
    return (
      <Modal title={title}
        visible={visible}
        width={600}
        ...
        footer={[
          ...
          <Button key="submit" type="primary" size="large" loading={this.state.loading} onClick={onOk}>
            通过
          </Button>
        ]}
      >
          ...
          <Tabs defaultActiveKey="base">
            ...
            <TabPane tab="电池信息" key="base3">
              {batPakList.map((item, index) => {
                return (
                  <Forms {...item} form={form} key={index} value={record && record['batPakList'][item.field]} />
                )
              })}
            </TabPane>
          </Tabs>
