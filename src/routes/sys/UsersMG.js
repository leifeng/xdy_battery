...
function UsersMG({dispatch, usersMG}) {
  ...
  const searchFormProps = {
    ...
    forms: [
      ...
      { label: '到期日期', field: 'dueDate', type: 'DatePicker', setting: { showTime: true, format: 'YYYY-MM-DD HH:mm:ss' } },
  ...
  const modalcusProps = {
    ...
    modalForms: [
      {
        label: '账号', field: 'account', type: 'Input', rules: [
      ...
      {
        label: '多选', field: 'aaa', type: 'Checkbox', dic: [
          { name: '可用', value: 1 },
          { name: '不可用', value: 0 }
        ]
      },
